...
2. 选中一个带关键帧的图层，按下快捷键。时间轴切换为 AE 原生图形编辑器，同时在鼠标附近弹出深色工具浮窗；已经显示曲线时不会反向切换。
...
6. 再次按下同一个快捷键，或点击浮窗右上角 **×**，工具窗口关闭，时间轴返回图层滑条。焦点在工具窗口时也可按 Esc。

工具浮窗保留所有选择、预设、滑块和模板控件，取消独立曲线画布。拖动顶部标题区可移动位置，呼出位置会自动避开屏幕边缘。工具窗口不会因鼠标移出而关闭。为避免卡顿，面板静止时不轮询属性。打开或点击 **读取** 时自动选中当前图层带两帧以上的属性；编辑动作执行前重新验证目标。原生编辑后可点 **读取** 更新滑块。如果关键帧在操作排队期间被更改，本次操作会停止并刷新，避免误写。详见 [曲线预设与模板](docs/curve-presets.md) 和 [原生曲线切换说明](docs/native-graph-editor.md)。
...
- 原生面板诊断日志写入 `%APPDATA%\DalimaoCurves\native-graph.log`。工具窗口标题为 `Dalimao Curves · 鼠标浮窗`。深色自绘外观位于 `CurvePopupUI.inl`，曲线由 AE 原生编辑器显示。
